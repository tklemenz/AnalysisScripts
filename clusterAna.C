...
int dEdxRes(TString clusterFile)
{
...
//      pionresMax = electronres = electronresMax = separationpower = separationpowerMax = 0.;
  ...
  TFile *TreeFile = TFile::Open(clusterFile.Data());
  TTree *tree = (TTree*)gDirectory->Get("clusters");
...
//  TH1F *hdEdxPionMax       = new TH1F("hdEdxMax", "; d#it{E}/d#it{x} Q_{max} (a.u.); # counts", 60, 0, 120);
  ...
  std::vector<Cluster> *vecCluster = nullptr;
  EventHeader Header;
  tree->SetBranchAddress("cl", &vecCluster);
