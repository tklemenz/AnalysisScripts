...
//bool InsideEdge(float row, float pad, int Setting, float PadCutLow=42, float PadCutHigh=49)
//{
//  if (Setting == 0) {
//    float slope = 18./47.;
//    if (row >= 0 && row <= 47) {
//      if (pad < (PadCutLow + row*slope) || pad > (PadCutHigh + row*slope)) {return false;}
//      else if (pad >= (PadCutLow + row*slope) && pad <= (PadCutHigh + row*slope)) {return true;}
//    }
//    else if (row > 47) {
//      if (pad < (PadCutLow - 3 + row*slope) || pad > (PadCutHigh - 3 + row*slope)) {return false;}
//      else if (pad >= (PadCutLow - 3 + row*slope) && pad <= (PadCutHigh - 3 + row*slope)) {return true;}
//    }
//  }
//  else if (Setting == 1) {
//    PadCutLow -= 9;
...
//      if (pad < (PadCutLow -4 + row*slope) && pad <= (PadCutHigh - 4 + row*slope)) {return false;}
//      else if (pad >= (PadCutLow -4 + row*slope) && pad <= (PadCutHigh -4 + row*slope)) {return true;}
//    }
//    else {
//      if (pad < (PadCutLow -7 + row*slope) && pad <= (PadCutHigh - 7 + row*slope)) {return false;}
//      else if (pad >= (PadCutLow -7 + row*slope) && pad <= (PadCutHigh -7 + row*slope)) {return true;}
//    }
//  }
//}
...
//float getTruncatedMean(float low, float high, int type, std::vector<Cluster> clCont)
//{
//  std::vector<float> values;
...
int getdEdxResolutionO2(TString FileList, TString GainMap="", const char *OutputPath="/home/tom/myana/Results")
{
...
//  TFile *OutFile = new TFile(Form("%s/anaOut.root", OutputPath), "recreate");
...
//  TH2D *hQmaxEleRuns            = new TH2D("hQmaxEleRuns", "; Run; Q_{max}", 350,0,350,167,0,200);
  ...
  TGraphErrors *gPionMean = new TGraphErrors();
...
//  TProfile *hQmaxRowS1CorrProf  = new TProfile();
  ...
  for (int i = 0; i < numberofruns; ++i) {
    hQmaxPiSingleRun[i] = new TH1F(Form("hQmaxPiSingleRun_%i",i),"; Q_{max} Pi; Counts", 600,0,600);
    hQtotPiSingleRun[i] = new TH1F(Form("hQtotPiSingleRun_%i",i),"; Q_{tot} Pi; Counts", 600,0,600);
    hQmaxEleSingleRun[i] = new TH1F(Form("hQmaxEleSingleRun_%i",i),"; Q_{max} Ele; Counts", 600,0,600);
    hQtotEleSingleRun[i] = new TH1F(Form("hQtotEleSingleRun_%i",i),"; Q_{tot} Ele; Counts", 600,0,600);
  }
  ...
  TH1F *hMPVQmaxPi[numberofruns];
  TH1F *hMPVQtotPi[numberofruns];
  TH1F *hMPVQmaxEle[numberofruns];
  TH1F *hMPVQtotEle[numberofruns];
  for (int i = 0; i < numberofruns; ++i) {
    MPVfitQmaxEleSingleRun[i] = new TF1(Form("MPVfitQmaxEleSingleRun_%i",i),"landau");
    MPVfitQtotEleSingleRun[i] = new TF1(Form("MPVfitQtotEleSingleRun_%i",i),"landau");
    MPVfitQmaxPiSingleRun[i] = new TF1(Form("MPVfitQmaxPiSingleRun_%i",i),"landau");
    MPVfitQtotPiSingleRun[i] = new TF1(Form("MPVfitQtotPiSingleRun_%i",i),"landau");
    hMPVQmaxPi[i] = new TH1F(Form("MPVQmaxPi_%i",i),"; MPV",600,0,600);
    hMPVQtotPi[i] = new TH1F(Form("MPVQtotPi_%i",i),"; MPV",600,0,600);
    hMPVQmaxEle[i] = new TH1F(Form("MPVQmaxEle_%i",i),"; MPV",600,0,600);
    hMPVQtotEle[i] = new TH1F(Form("MPVQtotEle_%i",i),"; MPV",600,0,600);
  }

  TGraph *gQmaxPi = new TGraph();
  TGraph *gQtotPi = new TGraph();
  TGraph *gQmaxEle = new TGraph();
  TGraph *gQtotEle = new TGraph();
...
/*========================================== CUT ================================================*/
          ...
          if (!InsideEdge(row, cpad, setting, nEdgeCut)){++ncledge;}
        }
...
//    }
    ...
    gPionMean->SetPointError(runcounter,0,pionsigmaTot);
...
//    hdEdxPionTot[runcounter]->Reset();
    ...
    if (hQmaxEleSingleRun[runcounter]->GetEntries() > 0) {
      hQmaxEleSingleRun[runcounter]->Fit(Form("MPVfitQmaxEleSingleRun_%i",runcounter),"NQ0","",0,600);
    }
    float QmaxEleMPV = MPVfitQmaxEleSingleRun[runcounter]->GetParameter(1);
    float QmaxEleMPVSigma = MPVfitQmaxPiSingleRun[runcounter]->GetParameter(2);

    if (hQtotEleSingleRun[runcounter]->GetEntries() > 0) {
      hQtotEleSingleRun[runcounter]->Fit(Form("MPVfitQtotEleSingleRun_%i",runcounter),"NQ0");
    }
    float QtotEleMPV = MPVfitQtotEleSingleRun[runcounter]->GetParameter(1);

    if (hQmaxPiSingleRun[runcounter]->GetEntries() > 0) {
      hQmaxPiSingleRun[runcounter]->Fit(Form("MPVfitQmaxPiSingleRun_%i",runcounter),"NQ0");
    }
    float QmaxPiMPV = MPVfitQmaxPiSingleRun[runcounter]->GetParameter(1);

    if (hQtotPiSingleRun[runcounter]->GetEntries() > 0) {
      hQtotPiSingleRun[runcounter]->Fit(Form("MPVfitQtotPiSingleRun_%i",runcounter),"NQ0");
    }
    float QtotPiMPV = MPVfitQtotPiSingleRun[runcounter]->GetParameter(1);

    hMPVQmaxEle[runcounter]->SetLineColor(kRed);
    hMPVQtotEle[runcounter]->SetLineColor(kRed);
    hMPVQmaxPi[runcounter]->SetLineColor(kRed);
    hMPVQtotPi[runcounter]->SetLineColor(kRed);

    for (int i = 0; i <= 5000 ; ++i) {
      hMPVQmaxEle[runcounter]->Fill(QmaxEleMPV);
      hMPVQtotEle[runcounter]->Fill(QtotEleMPV);
      hMPVQmaxPi[runcounter]->Fill(QmaxPiMPV);
      hMPVQtotPi[runcounter]->Fill(QtotPiMPV);
    }

    gQmaxPi->SetPoint(runcounter,runNr,QmaxPiMPV);
//    gQmaxPi->SetPointError(runcounter,0,QmaxEleMPVSigma);
    gQtotPi->SetPoint(runcounter,runNr,QtotPiMPV);
    gQmaxEle->SetPoint(runcounter,runNr,QmaxEleMPV);
    gQtotEle->SetPoint(runcounter,runNr,QtotEleMPV);
...
//  OutFile->Close();
  ...
  TFile *g = TFile::Open(Form("%s/anaOut.root", OutputPath), "recreate");
  g->WriteObject(gPionMean, "gPiondEdxMean");
  g->WriteObject(gQmaxPi, "gQmaxPiMPV");
  g->WriteObject(gQtotPi, "gQtotPiMPV");
  g->WriteObject(gQmaxEle, "gQmaxEleMPV");
  g->WriteObject(gQtotEle, "gQtotEleMPV");
  for (int i = 0; i < numberofruns; ++i) {
    g->WriteObject(hQmaxEleSingleRun[i], Form("hQmaxEleSingleRun_%i",i));
    g->WriteObject(hMPVQmaxEle[i], Form("MPVQmaxEle_%i",i));
    g->WriteObject(hQtotEleSingleRun[i], Form("hQtotEleSingleRun_%i",i));
    g->WriteObject(hMPVQtotEle[i], Form("MPVQtotEle_%i",i));
    g->WriteObject(hQmaxPiSingleRun[i], Form("hQmaxPiSingleRun_%i",i));
    g->WriteObject(hMPVQmaxPi[i], Form("MPVQmaxPi_%i",i));
    g->WriteObject(hQtotPiSingleRun[i], Form("hQtotPiSingleRun_%i",i));
    g->WriteObject(hMPVQtotPi[i], Form("MPVQtotPi_%i",i));
  }
