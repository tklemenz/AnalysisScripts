...
void AnalyseAsymmetry(TString FileList, TString GainMap="", const char *OutputPath="/home/tom/myana/Results/AnalysedEdxAsymmetry")
{
  EventHeader Header;
...
*/
  ...
  TH1F *hQmaxPiLow[nRuns];
  TH1F *hQmaxPiHigh[nRuns];
  TH1F *hQtotPiLow[nRuns];
  TH1F *hQtotPiHigh[nRuns];
  TH1F *hQmaxEleLow[nRuns];
  TH1F *hQmaxEleHigh[nRuns];
  TH1F *hQtotEleLow[nRuns];
  TH1F *hQtotEleHigh[nRuns];
  ...
  for (int i = 0; i < nRuns; ++i) {
    ...
    hQmaxPiLow[i]    = new TH1F(Form("hQmaxPiLow_%i", i),"; Q_{max low} [ADC counts]; Counts #pi",400,0,400);
    hQmaxPiHigh[i]   = new TH1F(Form("hQmaxPiHigh_%i", i),"; Q_{max high} [ADC counts]; Counts #pi",400,0,400);
    hQtotPiLow[i]    = new TH1F(Form("hQtotPiLow_%i", i),"; Q_{tot low} [ADC counts]; Counts #pi",600,0,600);
    hQtotPiHigh[i]   = new TH1F(Form("hQtotPiHigh_%i", i),"; Q_{tot high} [ADC counts]; Counts #pi",600,0,600);
    hQmaxEleLow[i]   = new TH1F(Form("hQmaxPiLow_%i", i),"; Q_{max low} [ADC counts]; Counts e",400,0,400);
    hQmaxEleHigh[i]  = new TH1F(Form("hQmaxPiHigh_%i", i),"; Q_{max high} [ADC counts]; Counts e",400,0,400);
    hQtotEleLow[i]   = new TH1F(Form("hQtotPiLow_%i", i),"; Q_{tot low} [ADC counts]; Counts e",600,0,600);
    hQtotEleHigh[i]  = new TH1F(Form("hQtotPiHigh_%i", i),"; Q_{tot high} [ADC counts]; Counts e",600,0,600);
  }
...
*/
        ...
        for (auto trackObject : *vecEvent) {
          ...
          for (auto &clusterObject : clCont) {															// make cuts on cluster properties
            ...
            /// comment 'if' and 'else if' statement
            if (setting == 0) {
          ...
          if (isok == true){
            ...
            dEdxTot = trackObject.getTruncatedMean(runNr, .0, .7, 1, false, false, false, hCheckExclude, 1, 2);
            dEdxMax = trackObject.getTruncatedMean(runNr, .0, .7, 0, false, false, false, hCheckExclude, 1, 2);
...
*/
      if (hdEdxPionTot[runcounter]->GetEntries() > 0 && hdEdxEleTot[runcounter]->GetEntries() > 0){
        ...
        hdEdxEleMax[runcounter]->SetLineColor(kRed);
        hdEdxPionMax[runcounter]->SetLineColor(kBlue);
...
/*==============================================================================================================================================
 *==============================================================================================================================================
 *========================  look at tracks right and left from the mean dEdx  ==================================================================
 *==============================================================================================================================================
 *==============================================================================================================================================
*/
        CherenkovValue = 0;
    ...
    /*=============================================== Loop over events =========================================================*/
            ...
            if (isok == true){
              ...
              if (CherenkovValue < CherCutLow) {
                ...
                for (auto &clusterObject : clCont) {
                  ...
                  if (dEdxTot <= pionmeanTot) {
                    hQmaxPiLow[runcounter]->Fill(Qmax);
                    hQtotPiLow[runcounter]->Fill(Qtot);
                  }
                  else if (dEdxTot > pionmeanTot) {
                    hQmaxPiHigh[runcounter]->Fill(Qmax);
                    hQtotPiHigh[runcounter]->Fill(Qtot);
                  }
                }
              }
                ...
                for (auto &clusterObject : clCont) {
                  ...
                  if (dEdxTot <= electronmeanTot) {
                    hQmaxEleLow[runcounter]->Fill(Qmax);
                    hQtotEleLow[runcounter]->Fill(Qtot);
                  }
                  else if (dEdxTot > electronmeanTot) {
                    hQmaxEleHigh[runcounter]->Fill(Qmax);
                    hQtotEleHigh[runcounter]->Fill(Qtot);
                  }
                }
              }
      ...
      hQmaxPiLow[runcounter]->SetLineColor(kGreen);
      hQmaxEleLow[runcounter]->SetLineColor(kGreen);
      hQtotPiLow[runcounter]->SetLineColor(kGreen);
      hQtotEleLow[runcounter]->SetLineColor(kGreen);

      hQmaxPiHigh[runcounter]->SetLineColor(kBlack);
      hQmaxEleHigh[runcounter]->SetLineColor(kBlack);
      hQtotPiHigh[runcounter]->SetLineColor(kBlack);
      hQtotEleHigh[runcounter]->SetLineColor(kBlack);

      hQmaxPiLow[runcounter]->Scale(1/hQmaxPiLow[runcounter]->GetEntries());
      hQmaxEleLow[runcounter]->Scale(1/hQmaxEleLow[runcounter]->GetEntries());
      hQtotPiLow[runcounter]->Scale(1/hQtotPiLow[runcounter]->GetEntries());
      hQtotEleLow[runcounter]->Scale(1/hQtotEleLow[runcounter]->GetEntries());

      hQmaxPiHigh[runcounter]->Scale(1/hQmaxPiHigh[runcounter]->GetEntries());
      hQmaxEleHigh[runcounter]->Scale(1/hQmaxEleHigh[runcounter]->GetEntries());
      hQtotPiHigh[runcounter]->Scale(1/hQtotPiHigh[runcounter]->GetEntries());
      hQtotEleHigh[runcounter]->Scale(1/hQtotEleHigh[runcounter]->GetEntries());
...
//    OutFile->WriteObject(TrackAna, "TrackAna");
  TFile *g = new TFile(Form("%s/anaAsymmetry.root", OutputPath), "recreate");

  for (int i = 0; i < nRuns; ++i) {
    g->WriteObject(hdEdxPionTot[i], Form("hdEdxPionTot_%i",i));
    g->WriteObject(hdEdxEleTot[i], Form("hdEdxEleTot_%i",i));
    g->WriteObject(hdEdxPionMax[i], Form("hdEdxPionMax_%i",i));
    g->WriteObject(hdEdxEleMax[i], Form("hdEdxEleMax_%i",i));
    g->WriteObject(hQmaxPiLow[i], Form("hQmaxPiLow_%i", i));
    g->WriteObject(hQmaxPiHigh[i], Form("hQmaxPiHigh_%i", i));
    g->WriteObject(hQmaxEleLow[i], Form("hQmaxEleLow_%i", i));
    g->WriteObject(hQmaxEleHigh[i], Form("hQmaxEleHigh_%i", i));
    g->WriteObject(hQtotPiLow[i], Form("hQtotPiLow_%i", i));
    g->WriteObject(hQtotPiHigh[i], Form("hQtotPiHigh_%i", i));
    g->WriteObject(hQtotEleLow[i], Form("hQtotEleLow_%i", i));
    g->WriteObject(hQtotEleHigh[i], Form("hQtotEleHigh_%i", i));

    delete hQmaxPiLow[i], hQmaxPiHigh[i], hQmaxEleLow[i], hQmaxEleHigh[i], hQtotPiLow[i], hQtotPiHigh[i], hQtotEleLow[i], hQtotEleHigh[i];
  }
