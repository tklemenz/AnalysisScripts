...
void dEdxCorrected_HVB_runs_diff_p(TString FileList, TString GainMap="", const char *OutputPath="/home/tom/myana/Results")
{
  ...
  const int nbeamMomentum = 5;
  TH1F *hdEdxEleTotAll[nbeamMomentum];
  TH1F *hdEdxPionTotAll[nbeamMomentum];
  ...
  for (int i = 0; i < nbeamMomentum; ++i) {
    hdEdxEleTotAll[i]      = new TH1F(Form("hdEdxEleTotAll_momentum_%i", -(i+1)), "; d#it{E}/d#it{x} (a.u.); Counts", 200, 0, 15);
    hdEdxPionTotAll[i]     = new TH1F(Form("hdEdxTotAll_momentum_%i", -(i+1)), "; d#it{E}/d#it{x} (a.u.); Counts", 200, 0, 15);
  }
...
/*=============================================== Loop over events =========================================================*/
    ...
    for (int iev=0; iev<tree->GetEntriesFast(); ++iev){
      ...
      runNr = Header.run;
      beamMomentum = -Header.beamMomentum - 1;
    ...
    for (int iev=0; iev<tree->GetEntriesFast(); ++iev){
      ...
      for (auto trackObject : *vecEvent) {
        ...
        if (isok == true){
          ...
          if (CherenkovValue < CherCutLow){
            hdEdxPionTotAll[beamMomentum]->Fill(dEdxTot);
          }
          else if (CherenkovValue > CherCutHigh){
            hdEdxEleTotAll[beamMomentum]->Fill(dEdxTot);
          }
  ...
  TCanvas *dEdx[nbeamMomentum];
  TF1 *pionfit[nbeamMomentum];
  TF1 *electronfit[nbeamMomentum];
  TPaveText *pave[nbeamMomentum];

  for (int i = 0; i < nbeamMomentum; ++i) {
    dEdx[i] = new TCanvas(Form("dEdx_momentum_%i", -(i+1)), Form("dEdx Qtot momentum %i", -(i+1)));
    pionfit[i] = new TF1(Form("pionfit_momentum_%i",i),"gaus",hdEdxPionTotAll[i]->GetXaxis()->GetXmin(),hdEdxPionTotAll[i]->GetXaxis()->GetXmax());
    electronfit[i] = new TF1(Form("electronfit_momentum_%i",i),"gaus",hdEdxEleTotAll[i]->GetXaxis()->GetXmin(),hdEdxEleTotAll[i]->GetXaxis()->GetXmax());
    ...
    hdEdxPionTotAll[i]->Fit(Form("pionfit_momentum_%i", i),"","",hdEdxPionTotAll[i]->GetXaxis()->GetBinLowEdge(bin1),hdEdxPionTotAll[i]->GetXaxis()->GetBinUpEdge(bin2));
    GetBinMinMax(hdEdxEleTotAll[i],frac,bin1,bin2);
    hdEdxEleTotAll[i]->Fit(Form("electronfit_momentum_%i",i),"","",hdEdxEleTotAll[i]->GetXaxis()->GetBinLowEdge(bin1),hdEdxEleTotAll[i]->GetXaxis()->GetBinUpEdge(bin2));

    //alternative fit
    //hdEdxPionTotAll[i]->Fit(Form("pionfit_momentum_%i",i));
    //hdEdxEleTotAll[i]->Fit(Form("electronfit_momentum_%i",i));


//    hdEdxEleTotAll[i]->GetFunction(Form("electronfit_momentum_%i",i))->SetLineColor(kRed);
//    hdEdxPionTotAll[i]->GetFunction(Form("pionfit_momentum_%i",i))->SetLineColor(kBlue);
...
*/
    ...
    pave[i]->AddText(Form("Beam momentum: %i GeV/#it{c}", -(i+1)));
    ...
    dEdx[i]->Update();
    dEdx[i]->Print(Form("%s/dEdxQtot_merged_momentum_%i.png", OutputPath, -(i+1)));
  }

  TCanvas *cdEdxMomenta = new TCanvas("cdEdxMomenta","dE/dx for HV setting B at different beam momenta");
  cdEdxMomenta->Divide(2,3);
  cdEdxMomenta->cd(1);
  hdEdxPionTotAll[0]->Draw();
  hdEdxEleTotAll[0]->Draw("same");
  pave[0]->Draw("same");
  dEdx[0]->Update();
  cdEdxMomenta->cd(2);
  hdEdxPionTotAll[1]->Draw();
  hdEdxEleTotAll[1]->Draw("same");
  pave[1]->Draw("same");
  dEdx[1]->Update();
  cdEdxMomenta->cd(3);
  hdEdxPionTotAll[2]->Draw();
  hdEdxEleTotAll[2]->Draw("same");
  pave[2]->Draw("same");
  dEdx[2]->Update();
  cdEdxMomenta->cd(4);
  hdEdxPionTotAll[3]->Draw();
  hdEdxEleTotAll[3]->Draw("same");
  pave[3]->Draw("same");
  dEdx[3]->Update();
  cdEdxMomenta->cd(5);
  hdEdxPionTotAll[4]->Draw();
  hdEdxEleTotAll[4]->Draw("same");
  pave[4]->Draw("same");
  dEdx[4]->Update();
  cdEdxMomenta->SaveAs(Form("%s/MergedHVB_BeamMomenta.pdf",OutputPath));

  //TrackAna->Fill();
  g->WriteObject(cdEdxMomenta, "dEdxResults");
  g->WriteObject(hQmaxPiRuns, "hQmaxPiRuns");
  g->WriteObject(hMPVRuns, "hMPVRuns");
  g->WriteObject(hChisquareMPVfit, "hChisquareMPVfit");
  for (int i = 0; i < nbeamMomentum; ++i) {
    g->WriteObject(hdEdxPionTotAll[i], Form("hdEdxPiTot_momentum_%i", -(i+1)));
    g->WriteObject(hdEdxEleTotAll[i], Form("hdEdxEleTot_momentum_%i", -(i+1)));
  }
  delete g;
}
