...
/*
float getTrackOffset(TrackTPC track)
{
  float x = track.getX();
  float y = track.getY();
...
float getTrackSlope(TrackTPC track)
{
...
int compareSimvsReco_oldO2(TString FileList, TString DataType="sim", TString GainMap="", const char *Filename="compareSimvsReco.root", const char *OutputPath="/home/tom/myana/Results/compareSimvsReco")
{
...
*/
  ...
  TH1F *hdEdxEleTotAll          = new TH1F("hdEdxEleTotAllSim", "; d#it{E}/d#it{x} (a.u.); Counts", 200, 0, 400);
  TH1F *hdEdxPionTotAll         = new TH1F("hdEdxTotAllSim", "; d#it{E}/d#it{x} (a.u.); Counts", 200, 0, 400);

  TH1F *hdEdxEleMax             = new TH1F("hdEdxEleMaxSim", "; d#it{E}/d#it{x} Q_{max} (a.u.); Counts", 100, 0, 120);
  TH1F *hdEdxPionMax            = new TH1F("hdEdxMaxSim", "; d#it{E}/d#it{x} Q_{max} (a.u.); Counts", 100, 0, 120);
  ...
  TH1D *hTrackResiduals         = new TH1D("hTrackResidualsSim", "; Y [cm]; Counts", 100,0,10);
  TH1D *hEvalX                  = new TH1D("hEvalXsim", "; Y [cm]; Counts", 100,-10,10);
  TH1D *hyClus                  = new TH1D("hyClussim", "; Y [cm]; Counts", 500,-50,50);
  TH1D *hxClus                  = new TH1D("hxClussim", "; X [cm]; Counts", 100,0,50);
  TH1D *hCpadMean               = new TH1D("hCpadMeansim", "; cPad; Counts", 50,0,50);
  ...
  TH2D *hCheckExclude           = new TH2D("CheckExcludeSim", "; Row; Pad", 63,0,63,100,-50,50);

  TH1D *hnClTrMeanTot           = new TH1D("NClTrMeanTotSim",";NClTrMean",70,0,70);
  TH1D *hnClTrMeanMax           = new TH1D("NClTrMeanMaxSim",";NClTrMean",70,0,70);
  ...
  TH2D *hnTruncVsnClTot         = new TH2D("nTruncVsnClTotSim","; nCl; nClTruncUsedTot",70,0,70,70,0,70);
  TH2D *hnTruncVsnClMax         = new TH2D("nTruncVsnClMaxSim","; nCl; nClTruncUsedMax",70,0,70,70,0,70);

  TH2D *hdEdxNCl2D              = new TH2D("hdEdxNCl2DSim","; nCl; dE/dx",63,0,63,800,0,200);
...
/*========================================== CUT ================================================*/
        ...
        if (isok == true){																			// track accepted
          ...
          if (setting == 0) {
            dEdxTot = trackObject.getTruncatedMean(runNr, .0, .7, 1, false, false, true, hCheckExclude, 1, 0, &nClTruncTot, nullptr, nullptr);//hTruncTrack[eventcounter], hChargeDistTrack[eventcounter]);
            dEdxMax = trackObject.getTruncatedMean(runNr, .0, .7, 0, false, false, true, hCheckExclude, 1, 0, &nClTruncMax, nullptr, nullptr);//hTruncTrack[eventcounter], hChargeDistTrack[eventcounter]);
          ...
          for (auto &clusterObject : clCont) {															// loop over clusters
            ...
            float xClus = 0.375 + 0.75 * row;

            float residual = yClus - evalX(trackObject, xClus);
...
//  delete f;
  ...
  return 0;
    ...
    TH1F *hdEdxEleTotAll          = new TH1F("hdEdxEleTotAllReco", "; d#it{E}/d#it{x} (a.u.); Counts", 2000, 0, 4000);
    TH1F *hdEdxPionTotAll         = new TH1F("hdEdxTotAllReco", "; d#it{E}/d#it{x} (a.u.); Counts", 2000, 0, 4000);

    TH1F *hdEdxEleMax             = new TH1F("hdEdxEleMaxReco", "; d#it{E}/d#it{x} Q_{max} (a.u.); Counts", 1000, 0, 1200);
    TH1F *hdEdxPionMax            = new TH1F("hdEdxMaxReco", "; d#it{E}/d#it{x} Q_{max} (a.u.); Counts", 1000, 0, 1200);
    ...
    TH1D *hTrackResiduals         = new TH1D("hTrackResidualsReco", "; Track residuals; Counts", 150,-1.5,1.5);
    TH1D *hTrackResidualsTime     = new TH1D("hTrackResidualsTimeReco","; Track residuals t-dir; Counts", 1000,-50,50);
    TH1D *hEvalX                  = new TH1D("hEvalXReco", "; Y_{Track} [cm]; Counts", 200,0,50);
    TH1D *hyClus                  = new TH1D("hyClusReco", "; Y_{Cluster} [cm]; Counts", 200,0,50);
    TH1D *hxClus                  = new TH1D("hxClusReco", "; X_{Cluster} [cm]; Counts", 100,80,130);
    ...
    TH1F *hRelPadPos              = new TH1F("hRelPadPos",";relative pad position;Counts",400,-2,2);
  ...
  //  TH2D *hQmaxEleRuns            = new TH2D("hQmaxEleRuns", "; Run; Q_{max}", 350,0,350,167,0,200);
    ...
    TH2D *hCheckExclude           = new TH2D("CheckExcludeReco", "; Row; Pad", 63,0,63,100,-50,50);

    TH1D *hnClTrMeanTot           = new TH1D("NClTrMeanTotReco",";NClTrMean",70,0,70);
    TH1D *hnClTrMeanMax           = new TH1D("NClTrMeanMaxReco",";NClTrMean",70,0,70);
    ...
    TH2D *hnTruncVsnClTot         = new TH2D("nTruncVsnClTotReco","; nCl; nClTruncUsedTot",70,0,70,70,0,70);
    TH2D *hnTruncVsnClMax         = new TH2D("nTruncVsnClMaxReco","; nCl; nClTruncUsedMax",70,0,70,70,0,70);

    TH2D *hdEdxNCl2D              = new TH2D("hdEdxNCl2DReco","; nCl; dE/dx",63,0,63,800,0,200);
  ...
  //*========================================== CUT ================================================
          ...
          if (isok == true){																			// track accepted
            ...
            if (setting == 0) {
              dEdxTot = trackObject.getTruncatedMean(runNr, .0, .7, 1, true, true, true, hCheckExclude, 1, 2, &nClTruncTot, nullptr, nullptr);// hTruncTrack[eventcounter], hChargeDistTrack[eventcounter]);
              dEdxMax = trackObject.getTruncatedMean(runNr, .0, .7, 0, true, true, true, hCheckExclude, 1, 2, &nClTruncMax, nullptr, nullptr);// hTruncTrack[eventcounter], hChargeDistTrack[eventcounter]);
            ...
            for (auto &clusterObject : clCont) {	// loop over clusters
              ...
              //Wie bekomme ich den cluster daneben?
              ...
              float yClus = posGlob.Y()-0.2+relPadPos*0.4;  // relPadPos 0 and 1 == pad edges
              //float yClus = posGlob.Y()+relPadPos*0.4;    // relPadPos 0 == pad centre
    ...
    TProfile *pQmaxNeighboringRowReco    = (TProfile*)hQmaxNeighboringRowLeft->ProfileX();
    ...
    //OutFile->WriteObject(pQNeighboringRowRight, "pQNeighboringRowRight");
    OutFile->Close();
