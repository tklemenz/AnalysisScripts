...
void getClusters(const char *filename, Int_t dEdxExcludeRows=2){
  ...
  //TFile *fGain = new TFile("~/GEMtest/code/files/GainMap_2014.root");
  TFile *fGain = new TFile("~/GEMtest/code/myana/GainMap.root");
  ...
  Int_t excludeEdge = 1;//(dEdxExcludeRows/10)%10;
  //dEdxExcludeRows%=10;
...
/*========================================== CUT ================================================*/
	  ...
	  for (int icl = 0; icl < ncl; ++icl){															// make cuts on cluster properties
	    ...
	    const int cpadint = (cl->GetCPad())+0.5;
	    const double cpad = cl->GetCPad();
	    UShort_t timeMax = cl->GetTimeMax();
...
/*========================================== CUT ================================================*/
		...
		if (fGainMap){
	      gainMapCorr = fGainMap->GetBinContent(row+6,cpadint+60);
